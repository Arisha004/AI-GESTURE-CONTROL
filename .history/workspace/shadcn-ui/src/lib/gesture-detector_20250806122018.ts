...
export const detectGesture = (landmarks: Landmarks): string => {
  ...
  // Finger tips close to their respective knuckles (indicating curl)
  ...
  // Thumb tip close to index knuckle (curled inward)
  calculateDistance(thumbTip, indexKnuckle) < 0.4 * indexWristDist &&

  // Finger angles indicating curl
  indexAngle > 70 &&
  middleAngle > 70 &&
  ringAngle > 70 &&
  pinkyAngle > 70 &&

  // All finger tips are close together (tight fist)
  calculateDistance(indexTip, middleTip) < 0.15 * middleWristDist &&
  calculateDistance(middleTip, ringTip) < 0.15 * middleWristDist &&
  calculateDistance(ringTip, pinkyTip) < 0.15 * middleWristDist
) {
