...
export const detectGesture = (landmarks: Landmarks): string => {
  ...
  if (detectedGesture === lastGesture || (detectedGesture !== 'No gesture detected' && lastGesture === 'No gesture detected')) {
    // Same gesture detected again or new valid gesture detected when there was none
    gestureCounter++;
    if (gestureCounter >= STABILITY_THRESHOLD) {
      // Only update lastGesture if we've seen the same gesture for several frames
