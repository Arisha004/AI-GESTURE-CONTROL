...
export const detectGesture = (landmarks: Landmarks): string => {
  ...
  middleWristDist < 0.4 * calculateDistance(middleKnuckle, wrist) &&
  ringWristDist < 0.4 * calculateDistance(ringKnuckle, wrist) &&
  pinkyWristDist < 0.4 * calculateDistance(pinkyKnuckle, wrist) &&
...
const STABILITY_THRESHOLD = 5; // Lower this if needed
