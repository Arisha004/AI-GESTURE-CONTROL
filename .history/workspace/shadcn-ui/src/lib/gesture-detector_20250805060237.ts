...
export const detectGesture = (landmarks: Landmarks): string => {
  ...
  indexWristDist > 0.7 * calculateDistance(indexKnuckle, wrist) &&
  middleWristDist > 0.7 * calculateDistance(middleKnuckle, wrist) &&
  ringWristDist > 0.7 * calculateDistance(ringKnuckle, wrist) &&
  pinkyWristDist > 0.7 * calculateDistance(pinkyKnuckle, wrist) &&
  thumbWristDist > 0.6 * calculateDistance(landmarks[1], wrist) &&
  indexAngle < 50 &&
  middleAngle < 50 &&
  ringAngle < 50 &&
  pinkyAngle < 50 &&
  // Spread fingers (not tight together)
  calculateDistance(indexTip, middleTip) > 0.25 * middleWristDist &&
  calculateDistance(middleTip, ringTip) > 0.25 * ringWristDist &&
  calculateDistance(ringTip, pinkyTip) > 0.25 * pinkyWristDist
) {
  ...
  indexWristDist > 0.7 * calculateDistance(indexKnuckle, wrist) &&
  middleWristDist > 0.7 * calculateDistance(middleKnuckle, wrist) &&
  ringWristDist < 0.5 * calculateDistance(ringKnuckle, wrist) &&
  pinkyWristDist < 0.5 * calculateDistance(pinkyKnuckle, wrist) &&
  // Thumb must be more relaxed, closer to palm
  thumbWristDist < 0.6 * Math.max(indexWristDist, middleWristDist) &&
  // Ensure index and middle are close (aligned)
  Math.abs(indexTip[1] - middleTip[1]) < 0.15 * middleWristDist
) {
  ...
  indexKnuckleDist < 0.3 * indexWristDist &&
  middleKnuckleDist < 0.3 * middleWristDist &&
  ringKnuckleDist < 0.3 * ringWristDist &&
  pinkyKnuckleDist < 0.3 * pinkyWristDist &&
  calculateDistance(thumbTip, indexKnuckle) < 0.3 * indexWristDist &&
  indexAngle > 60 &&
  middleAngle > 60 &&
  ringAngle > 60 &&
  pinkyAngle > 60 &&
  // Tips should be near palm and somewhat close
  calculateDistance(indexTip, middleTip) < 0.1 * middleWristDist &&
  calculateDistance(middleTip, ringTip) < 0.1 * ringWristDist &&
  calculateDistance(ringTip, pinkyTip) < 0.1 * pinkyWristDist
) {
  detectedGesture = 'fist';
}
console.log("Detected Gesture (raw):", detectedGesture);
...
// Confirm gesture only after stability threshold
