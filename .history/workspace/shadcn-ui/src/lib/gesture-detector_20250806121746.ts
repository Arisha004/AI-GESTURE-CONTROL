 ...
 * Returns one of: 'thumbs_up', 'peace', 'closed_palm', 'fist', 'no_gesture'
 */
export const detectGesture = (landmarks: Landmarks): string => {
  ...
  // Check if finger tips are clustered (close together)
  const tipsAreClustered =
    calculateDistance(indexTip, middleTip) < 0.07 * middleWristDist &&
    calculateDistance(middleTip, ringTip) < 0.07 * ringWristDist &&
    calculateDistance(ringTip, pinkyTip) < 0.07 * pinkyWristDist;

  // Check if thumb is tucked (thumb tip close to palm center)
  const palmCenter = [
    (wrist[0] + middleKnuckle[0]) / 2,
    (wrist[1] + middleKnuckle[1]) / 2,
    (wrist[2] + middleKnuckle[2]) / 2
  ];
  const thumbIsTucked = calculateDistance(thumbTip, palmCenter) < 0.25 * calculateDistance(landmarks[1], wrist);
