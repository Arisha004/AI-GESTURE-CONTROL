...
export const detectGesture = (landmarks: Landmarks): string => {
  ...
  if (
    indexWristDist > 0.6 * calculateDistance(indexKnuckle, wrist) &&
    middleWristDist > 0.6 * calculateDistance(middleKnuckle, wrist) &&
    ringWristDist > 0.6 * calculateDistance(ringKnuckle, wrist) &&
    pinkyWristDist > 0.6 * calculateDistance(pinkyKnuckle, wrist) &&
    thumbWristDist > 0.4 * calculateDistance(landmarks[1], wrist) &&
    // Check that all fingers are roughly straight
    indexAngle < 45 &&
    middleAngle < 45 &&
    ringAngle < 45 &&
    pinkyAngle < 45 &&
    // Make sure all fingers are separated
