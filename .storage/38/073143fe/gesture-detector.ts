...
export const detectGesture = (landmarks: Landmarks): string => {
  ...
  // Apply stability logic - only change the gesture if we've seen it consistently
  if (detectedGesture === lastGesture) {
    ...
    gestureCounter = 1;
    // Don't update lastGesture yet - wait for stability
  }
  
  // If we've seen the same gesture for several frames, update our stable gesture
  if (gestureCounter >= STABILITY_THRESHOLD) {
    lastGesture = detectedGesture;
  }
  
  // Always return the stable gesture
  return lastGesture;
};