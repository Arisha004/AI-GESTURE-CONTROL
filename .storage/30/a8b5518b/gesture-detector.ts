...
export const detectGesture = (landmarks: Landmarks): string => {
  ...
  if (
    ...
    ringKnuckleDist < 0.5 * ringWristDist &&
    pinkyKnuckleDist < 0.5 * pinkyWristDist &&
    calculateDistance(thumbTip, landmarks[1]) < 0.5 * thumbWristDist &&
    Math.abs(indexTip[0] - middleTip[0]) > 20 // Fingers are spread apart
  ) {
    return 'peace';
  }
  ...
  if (
    ...
    ringKnuckleDist < 0.5 * ringWristDist &&
    pinkyKnuckleDist < 0.5 * pinkyWristDist &&
    thumbTip[0] > indexKnuckle[0] && // Thumb is to the right of index knuckle
    indexAngle > 70 &&
    middleAngle > 70 &&
    ringAngle > 70 &&
    pinkyAngle > 70
