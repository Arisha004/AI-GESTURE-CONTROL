...
// Approximate video width for scaling - will be updated during detection
let videoWidth = 640;
let lastDetectedGesture = '';
let gestureConfidence = 0;
const CONFIDENCE_THRESHOLD = 3; // Number of consistent detections required
...
/**
 * Sets the current video width for better scaling of gesture detection
 */
export const setVideoWidth = (width: number): void => {
  videoWidth = width;
};
 ...
 * Includes gesture stabilization to prevent flickering
 */
export const detectGesture = (landmarks: Landmarks): string => {
  if (!landmarks || landmarks.length !== 21) {
    gestureConfidence = 0;
    lastDetectedGesture = 'No gesture detected';
  ...
  // For thumbs up: thumb is extended while other fingers are curled
  if (
  ...
  if (
    // Ensure all fingers are extended away from palm
    ...
    // Check that all fingers are more straight than in a fist
    indexAngle < 40 &&
    middleAngle < 40 &&
    ringAngle < 40 &&
    pinkyAngle < 40 &&
    // Check that fingertips are further from wrist than knuckles
    indexWristDist > 1.2 * calculateDistance(indexKnuckle, wrist) &&
    middleWristDist > 1.2 * calculateDistance(middleKnuckle, wrist) &&
    // Ensure this is not misclassified as a fist
    calculateDistance(indexTip, wrist) > 0.7 * videoWidth
  ) {
    return 'open_palm';
  }
  ...
  if (
    ...
    ringAngle > 70 &&
    pinkyAngle > 70 &&
    // All fingertips should be closer to palm than to wrist
    calculateDistance(indexTip, wrist) < 0.6 * videoWidth &&
    // Ensure this is not misclassified as open palm
  ...
  // Implement gesture stabilization to prevent flickering
  if (currentGesture === lastDetectedGesture) {
    gestureConfidence++;
  } else {
    // If we detect a new gesture, start building confidence
    if (gestureConfidence < CONFIDENCE_THRESHOLD) {
      // Not enough confidence in the new gesture yet, stick with the last one
      currentGesture = lastDetectedGesture;
      gestureConfidence++;
    } else {
      // We have a new gesture with enough confidence
      gestureConfidence = 1;
